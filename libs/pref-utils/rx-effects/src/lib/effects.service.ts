...
import { HookProps, OnDestroy$, toHook } from '@opis/cdk';
...
export class RxEffects implements OnDestroy, OnDestroy$ {
  ...
  readonly _hooks$ = new Subject<HookProps>();
   ...
   */
  untilDestroy() {
    return <V>(source: Observable<V>) => source.pipe(untilDestroyed(this as any));
  }
